...
@section('content')
                            ...
                            <div class="card-footer  text-center">
                                <div class="mb-3">
                                    <form method="post"
                                        ...
                                        class="row justify-content-around text-center mt-2" id="ratingForm">
                                        @csrf
                                        <div id="rating"></div>
                                        <input type="hidden" name="rating" id="ratingInput"
                                            value="{{ $userRatings->get($book->book_id) }}">

                                        <!-- Añade más campos o estilos según sea necesario -->
    ...
    <script>
        $(document).ready(function() {
            ...
            var currentRating = {{ $userRatings->get($book->book_id) ?? 0 }};

            $("#rating").rateYo({
                rating: currentRating, // Establece la calificación actual como valor inicial
                fullStar: true,
                onSet: function(rating, rateYoInstance) {
                    $("#ratingInput").val(rating); // Actualiza el valor del input oculto
                    $("#ratingForm")
                        .submit(); // Envía automáticamente el formulario al seleccionar las estrellas
                }
            });
        });
    </script>
