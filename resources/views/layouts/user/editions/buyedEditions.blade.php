            ...
            <div class="row row-cols-1 row-cols-md-3 pe-4">
                ...
                @else
                    @foreach ($books as $book)
                        <div class="card ms-3 border border-black rounded"
                            ...
                            89 433 160 658 185 122 14 495 5 614 -14z" />
                                ...
                                <div class="">
                                    <form method="post" action="{{ route('user.library.rate-book', ['id' => $book->book_id]) }}" class="row justify-content-around text-center mt-2">
                                        @csrf
                                        <select name="rating" class="form-select w-25 col-6 border-1 border-black rounded">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <option value="{{ $i }}" {{ $userRatings->get($book->book_id) == $i ? 'selected' : '' }}>
                                                    {{ $i }} ★
                                                </option>
                                            @endfor
                                        </select>
                                        <button type="submit" class="btn btn-primary col-6">Valorar</button>
                                    </form>
                                </div>
                            </div>
                        </div>
